...
import (
	...
	"github.com/byteplus-sdk/byteplus-sdk-golang/service/live"
)
...
func TestDescribePullToPushBandwidthData(t *testing.T) {
	...
	body, _ := json.Marshal(bodyMap)
	resp, statusCode, err := live.DefaultInstance.DescribePullToPushBandwidthData(nil, string(body))
	if err != nil {
		t.Logf("error occur %v", err)
	}
	res, _ := json.Marshal(resp)
...
func TestListStorageSpace(t *testing.T) {
	live.DefaultInstance.Client.SetAccessKey(testAk)
	live.DefaultInstance.Client.SetSecretKey(testSk)
	bodyMap := map[string]interface{}{
	...
	//打印请求参数
	t.Logf(string(body))
	resp, statusCode, err := live.DefaultInstance.ListStorageSpace(nil, string(body))
	if err != nil {
		t.Logf("error occur %v", err)
	}
	res, _ := json.Marshal(resp)
...
func TestDescribeLiveStorageSpaceData(t *testing.T) {
	live.DefaultInstance.Client.SetAccessKey(testAk)
	live.DefaultInstance.Client.SetSecretKey(testSk)
	bodyMap := map[string]interface{}{
	...
	body, _ := json.Marshal(bodyMap)
	resp, statusCode, err := live.DefaultInstance.DescribeLiveStorageSpaceData(nil, string(body))
	if err != nil {
		t.Logf("error occur %v", err)
	}
	res, _ := json.Marshal(resp)
