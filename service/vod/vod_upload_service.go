 ...
 * @return *response.VodParseUploadManifestResponse, int, error
 */
func (p *Vod) ParseUploadManifest(req *request.VodParseUploadManifestRequest) (*response.VodParseUploadManifestResponse, int, error) {
	query := url.Values{}
	form := url.Values{}
	marshaler := protojson.MarshalOptions{
		Multiline:       false,
	...
	respBody, status, err := p.Json("ParseUploadManifest", url.Values{}, jsonData)

	output := &response.VodParseUploadManifestResponse{}
	unmarshaler := protojson.UnmarshalOptions{
		DiscardUnknown: true,
	}
	errUnmarshal := unmarshaler.Unmarshal(respBody, output)
