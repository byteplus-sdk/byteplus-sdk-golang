 ...
 * @return *response.VodCreateHlsDecryptionKeyResponse, int, error
 */
func (p *Vod) CreateHlsDecryptionKey(req *request.VodCreateHlsDecryptionKeyRequest) (*response.VodCreateHlsDecryptionKeyResponse, int, error) {
	query := url.Values{}
	form := url.Values{}
	marshaler := protojson.MarshalOptions{
		Multiline:       false,
	...
	respBody, status, err := p.Query("CreateHlsDecryptionKey", query)

	output := &response.VodCreateHlsDecryptionKeyResponse{}
	unmarshaler := protojson.UnmarshalOptions{
		DiscardUnknown: true,
	}
	errUnmarshal := unmarshaler.Unmarshal(respBody, output)
